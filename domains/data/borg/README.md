...
- 2026-05-21: Deleted `parts/scripts.nix` — 58 lines of `mkBorgWrapper` /
  `mkListBackupsScript` / `mkRestoreScript` helper builders that `index.nix` never
  imported (its header still named the pre-migration path
  `domains/system/services/borg/`). `parts/` is gone with it. Landed inside the
  unrelated `5da97868`.
