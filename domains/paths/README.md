...
- 2026-07-06: Removed the `hwc.paths.user.shared` option + `HWC_SHARED_DIR` env export as part of retiring the `600_shared` NFS share (standardize on Syncthing for cross-machine sharing). Paths-only change; the NFS domain module, laptop automount, and server export were dropped in the same commit but live outside this domain.
