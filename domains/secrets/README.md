...
## Changelog
- 2026-08-12: Added `tailscale-authkey` (`parts/networking/tailscale-authkey.age`) — first secret in the new `networking/` category. Plaintext is a Tailscale **OAuth client secret** (`tskey-client-…`), not an auth key: client secrets never expire, where auth keys cap at 90 days, so the node can re-register itself unattended forever. The client is scoped to `tag:server`, so registrations come up as tagged devices, which are exempt from node-key expiry — the actual fix for the 2026-08-07 outage, where the untagged hwc-server hit the tailnet's 6-month expiry and dropped off the tailnet (SSH failed `Network is unreachable`; recovered over LAN at 192.168.0.97). Standard `root:secrets / 0440`, recipients = everyone (encrypted directly with `age` to the four recipient keys — laptop/server/xps/eric; no full rekey). Consumed by `services.tailscale.authKeyFile` on hwc-server via `machines/server/config.nix`, paired with `authKeyParameters = { preauthorized = true; ephemeral = false; }`.
