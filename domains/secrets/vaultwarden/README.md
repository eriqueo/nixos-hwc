...
| `image` | `docker.io/vaultwarden/server:1.35.4` | Container image (critical tier, Law 15 v12.4 — pinned) |
...
## Changelog
- 2026-07-06: Pinned the `image` default `docker.io/vaultwarden/server:latest` → `:1.35.4` (Law 15 v12.4 critical tier — password vault; set to the currently-running version so the switch is behavior-neutral). Follow-up fixed the pin comment that had eaten the `mkOption` closing brace.
