...
let
  chatScript = pkgs.writeScriptBin "ai-chat" ''
    ...
    class ChatSession:
        ...
        def setup_database(self):
            ...
            self.cursor.execute(
                'INSERT INTO conversations (started_at, model) VALUES (?, ?)',
        ...
        def save_message(self, role, content):
            ...
            self.cursor.execute(
                ...
                (self.conversation_id, datetime.now().isoformat(), role, content)
            )
        ...
        def query_ollama(self, prompt, stream=True):
            ...
            try:
                ...
                full_response = ""
                if stream:
                    print(f"{Colors.GREEN}{Colors.BOLD}Assistant:{Colors.ENDC} ", end="", flush=True)
                    ...
                    print(f"{Colors.GREEN}{Colors.BOLD}Assistant:{Colors.ENDC} {full_response}")
        ...
        def list_models(self):
            ...
            try:
                ...
                print(f"\n{Colors.CYAN}{Colors.BOLD}Available Models:{Colors.ENDC}")
                for model in models:
        ...
        def switch_model(self, model_name):
            ...
            try:
                ...
                if model_name in models:
                    ...
                    # Reset context when switching models
                    self.context = []
                    print(f"{Colors.GREEN}✓ Switched to model: {model_name}{Colors.ENDC}")
                else:
                    print(f"{Colors.RED}❌ Model not found: {model_name}{Colors.ENDC}")
                    print(f"{Colors.YELLOW}Available models: {', '.join(models)}{Colors.ENDC}")
...
{Colors.YELLOW}Chat Commands:{Colors.ENDC}
...
"""
        ...
        def show_history(self, n=10):
            ...
            if not messages:
                print(f"{Colors.YELLOW}No messages in history{Colors.ENDC}")
                return

            print(f"\n{Colors.CYAN}{Colors.BOLD}Last {len(messages)} Messages:{Colors.ENDC}\n")
        ...
        def run(self):
            """Main chat loop"""
            print(f"{Colors.BOLD}{Colors.BLUE}🤖 AI Chat Interface{Colors.ENDC}")
            print(f"{Colors.CYAN}Model: {self.model}{Colors.ENDC}")
            print(f"{Colors.CYAN}Type /help for commands, /quit to exit{Colors.ENDC}\n")
