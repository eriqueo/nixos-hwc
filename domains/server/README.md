...
## Changelog
- 2026-08-16: `native/ai/research-scout/` — three changes on the same day. `8f07d78b`
  (+38) wires the weekly-digest/suggestion output at email-sized bodies, paired with the
  notify schema's body cap coming off 4000 (see `domains/notifications/`); `6ec4a9bc`
  (+13) points item scoring at the self-hosted DX1 endpoint instead of a hosted model;
  `8ff93171` (+13/-3) drops `cs.LG` and `stat.ML` from the arXiv category seed — the two
  broadest categories were swamping the feed.
