...
## Changelog
- 2026-09-21: Law 12 refresh — `native/ai/hermes/parts/bootstrap/README.md`
  gained a `## Changelog` recording the 2026-05-29 `HermesDeployError` rewrite
  (parameter-property shorthand is a runtime `SyntaxError` under Node 22's
  `--experimental-strip-types`). Doc-only; no module here changed.
- 2026-09-19: `services/inbox-processor/` on hwc-server now writes to the vault's `_inbox/` (machine config `brainInboxPath`). It had kept writing to `inbox/`, which the vault reorganisation retired; the end-to-end test of the DX2 cleanup pass created that directory from nothing, which is how the drift was found. `brain-mcp`'s `append_to_inbox` tool still targets `inbox/` and is not changed here.
...
- 2026-09-04: `native/ai/hwc-control-bot/` slice 2 — `targets.researchScout = { enable; profile }` adds the Research Scout review lane to the registry (URL from `researchScout.port`, token mount, `research-scout.service` as a soft dependency). `native/ai/research-scout/` gained `controlTokenSecret` (new agenix `hwc-control-research-scout-token`, same shape and verification as the Lead Scout one), rendered as `RESEARCH_SCOUT_CONTROL_TOKEN_FILE` for its `/api/control/v1/reviews` routes only. Machine config enables the target for `llm_engineering_v1`.
