...
3. Restart: `sudo systemctl restart podman-frigate-v2.service`
...
- [ ] Archive old module
- [ ] Update server profile to use frigate-v2
...
# Start service
sudo systemctl start podman-frigate-v2.service

# Restart (after config changes)
sudo systemctl restart podman-frigate-v2.service

# Check status
sudo systemctl status podman-frigate-v2.service
```
...
```bash
# Container logs
podman logs frigate-v2
podman logs frigate-v2 --tail 100
podman logs frigate-v2 --follow

# Systemd service logs
journalctl -u podman-frigate-v2.service -f
```
...
diff domains/server/frigate-v2/config/config.yml \
     <(podman exec frigate-v2 cat /config/config.yml)
```
...
podman logs frigate-v2 --tail 50
```
...
podman exec frigate-v2 cat /config/config.yml | diff - config/config.yml
...
| Aspect | Old (frigate) | New (frigate-v2) |
|--------|---------------|------------------|
...
**Phase 4**: Cutover
- Switch server profile to frigate-v2
- Archive old module
