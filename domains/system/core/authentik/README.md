...
## Changelog
- 2026-08-28: **The role and its database ownership are declared, not scripted.**
  `parts/config.nix` used to `CREATE ROLE` and issue four GRANTs through `$PSQL` in
  `postgresql.postStart`. `$PSQL` is undefined in the generated post-start script
  and every line ended in `|| true`, so none of it ever ran — the `authentik` role
  exists on the live cluster only because someone created it by hand. Of the ten
  modules that wrote `postStart`, this was the **only** one whose dead code was
  load-bearing: a rebuilt cluster would have had the authentik database and no
  authentik role. It is now
  `services.postgresql.ensureUsers = [{ name = cfg.database.user; ensureDBOwnership = true; }]`,
  which reproduces live state exactly (authentik already owns its database) and
  makes the four grants redundant, since an owner's privileges are implicit. The
  dropped `PASSWORD 'placeholder'` is not a regression — authentik reaches Postgres
  over the podman bridge, and `pg_hba` matches `host all all 10.89.0.0/16 trust`
  before any md5 rule. Full 54-statement audit in
  `domains/data/databases/README.md` (`e82ca994`).
