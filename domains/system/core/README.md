...
- 2026-06-11: `5519f201` — `hwc.system.core.nixld.guiLibs` flag in system core replaces the duplicate desktop/gaming library list (companion to the parent-domain entry). All 5 toplevels byte-identical.
- 2026-06-09: Law 9/10 sweep (`aea7be73`) — option-declaring leaf modules under `core/` converted to directory modules via `git mv` (history-preserving, no behaviour change). Server tailnet rename (`22bbbb48`) swept references. Added sqlite CLI to the base bundle (`fb75c028`).
