...
- 2026-06-22: MCP — datax_* tools (DataX hub: board + gauntlet verdict + Firestore health); mail-triage write-path (tag-backed buckets, move/archive/delete); hwc_mail_triage cached email triage Result Contract; Result Contract emission from 6 server-health tools (+ Server hub) and 4 tile tools; hwc_tasks_* CRUD over Radicale CalDAV for claude.ai; hwc_calendar uses khalt's khal + Radicale calendars. Refinery — morning PR-review agent + workbench nightly review surface. Docs — sweep stale `tasq` client references → `todui`. Users — keep hwc-phone Termius SSH key in declarative authorized set.
