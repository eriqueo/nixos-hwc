...
## Changelog
- 2026-06-29: MCP surface grew significantly. `hwc_morning_brief` tool reads the rich `briefing.json` (1fe286df). `datax_*` tools: DataX hub goes live with board, gauntlet verdict and Firestore health (d76d72d8). `hwc_mail_triage` cached email triage (d77c45ff) plus mail-triage write-path with tag-backed buckets, move/archive/delete (39aca8be). All 6 server-health tools (c8971a2d) and the 4 tile tools (3ee73921) now emit a Universal Result Contract. `hwc_calendar` switched to khalt's `khal` + Radicale calendars (9437d2dc); `hwc_tasks_*` task CRUD over Radicale CalDAV for claude.ai (bd9a1cfc); sweep stale `tasq` → `todui` client references (abaaa5b5); workbench nightly-review surface + morning PR-review agent integration (344e53f0). `core/login` unifies the session bus by dropping `dbus-run-session` from `hyprStart` (ed836897); `core/` envfs FHS shim unbreaks Claude Cowork Bash on NixOS (2649d52c); `users/` keeps the hwc-phone Termius SSH key in the declarative authorized set (96bde13d).
