...
## Changelog
- 2026-09-14: Law-12 refresh for `core/` and `core/authentik/` — both now record
  the 2026-08-28 change replacing authentik's dead `postStart` `CREATE ROLE` +
  GRANT block with `ensureUsers` + `ensureDBOwnership`, and `core/README.md`'s
  Structure now lists the stray tracked `paths.nix.bak`. Docs only.
- 2026-09-10: `mounts/index.nix` — dropped `downloads`, `incomplete` and `blackhole` from the `media.directories` default, and dropped the `transcript-text` tmpfiles rule from the hot block. All four were pre-hot/cold-split leftovers: downloads have lived on `/mnt/hot` since the tiers were separated, no download client watches a blackhole, nothing ever wrote transcript-text (`transcripts.service` writes `/mnt/media/transcripts`), and all four had been empty for as long as the listings go back. **The directory was never the thing that persisted — this module was.** They were deleted from disk earlier the same day and the next `nixos-rebuild` recreated every one, which is the only reason the real producer got found. Worth keeping in mind for the next audit of this kind: a repo search will not find these. `media.directories` names them as BARE STRINGS joined to `cfg.media.path`, and the hot rule builds on `cfg.hot.path` — so neither `rg '/mnt/media/blackhole'` nor `rg 'media\.root}/'` can match, and both return a confident zero. Establish a directory's producer from the consuming end instead: `/etc/tmpfiles.d/00-nixos.conf` on the live host, plus the container mount table. Note also that this module and `domains/media/directories.nix` are two producers of media directory creation; that overlap is untouched here and is the thing to fix if these keep drifting.
