...
- 2026-07-27: Law 12 changelog backfill — the `parts/estimator-integration/`, `parts/migrations/`, and `parts/workflows/` READMEs were refreshed to catch up their changelogs with already-committed changes (tailnet sweep, migration add/remove, frigate HLS-link fix, retired monitoring workflows). Docs only; no functional change.
