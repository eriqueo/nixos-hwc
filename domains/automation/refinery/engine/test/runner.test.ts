...
import { Manifest } from "../src/contracts.js";
...
test("park-and-resume: a parked item resumes from its parked stage on next pass", async () => {
  ...
  const first = await runPass(item, manifest, firstGates, { store, clock: fixedClock });
  ...
  const second = await runPass(first.item, manifest, secondGates, {
    store,
    clock: fixedClock,
  });
...
test("rewind moves backward, sets pending, appends a 'rewound' history entry; re-runs on next pass", async () => {
  ...
  const replay = await runPass(rewound, manifest, replayGates, {
    store,
    clock: fixedClock,
  });
...
test("runPass throws UnknownGateError when manifest references a missing gate", async () => {
  ...
  await assert.rejects(
    () => runPass(item, manifest, gates, { store, clock: fixedClock }),
    /unregistered gate: g2/,
  );
});

test("runPass throws UnknownStageError when item.stage isn't in manifest", async () => {
  resetClock();
  const store = new InMemoryItemStore();
