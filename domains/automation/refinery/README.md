...
| `engine/src/shells/` | HTTP shell over the core: `http.ts` (routes — `/` Flow board, `/hopper`, `/nightly` Overnight, `/finished`, `/sr`, attention-first `/reviews`, `/review/:id` decision detail, `/reference`, per-item detail, + mutation handlers including exact-card review requeue), `render.ts` (Flow board = projects in state lanes; review cards show executive context/recommendation/action while technical evidence stays folded; plain form-posts, CSS-only interactivity), `serve.ts` (service entry). |
...
- 2026-08-29: **Reviews are now an executive decision surface.** `/reviews`
  ...
  collapsed. Cards link to a real `/review/:id` detail page with What happened,
