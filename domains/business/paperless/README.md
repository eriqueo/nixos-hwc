...
- 2026-08-28: `parts/config.nix` — deleted this module's eight dead `$PSQL`
  postStart statements (e82ca994). `$PSQL` is never assigned in the generated
  postgresql post-start script under the pinned 15.x module, and each line ended
  in `|| true`, so none of them ever ran; `paperless` holds 0 rows in
  `pg_class.relacl`, which is the measurement proving it. Not repaired, because
  the grantee is `eric` — a superuser who already owns the objects.
  `ensureDBOwnership` is deliberately NOT set here: the `paperless` database is
  owned by `postgres` while its objects are owned by `eric`, and setting it
  would rewrite live ownership to chase a tidier declaration. Full audit in
