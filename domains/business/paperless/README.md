...
- 2026-08-28: Removed the eight `$PSQL` GRANT statements from
  `parts/config.nix`'s `postStart` (`e82ca994`). None ever ran, and the proof
  is measurable: `paperless` holds 0 rows in `pg_class.relacl`. `$PSQL` is
  undefined in the generated postgresql post-start script and `|| true`
  swallowed each command-not-found. Deleted rather than repaired — `eric` owns
  this database's objects, so the grants were no-ops even had they fired.
  `ensureDBOwnership` was deliberately not added (the database is owned by
  `postgres`, its objects by `eric`). Rationale in
