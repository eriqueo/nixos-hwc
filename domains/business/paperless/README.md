...
- 2026-07-13: `paperless-receipts-mover` now **drains** the watched folder before
  exiting (up to 6 passes, 5s apart, waiting out files modified in the last 5
  seconds) and sets `startLimitIntervalSec = 0`. The `PathExistsGlob` unit
  re-fires while any matching file remains, so the old skip-and-exit on a
  still-syncing file looped the service straight into its start limit (9cebbb2f).
