...
- 2026-08-28: Deleted the eight-line `postStart` privilege block in
  `parts/config.nix`. None of those `GRANT` / `ALTER DEFAULT PRIVILEGES` lines had
  ever run — `$PSQL` is undefined in the generated postgresql post-start script and
  `|| true` swallowed the command-not-found. They were not restored because they
  were never load-bearing: paperless connects as `database.user` (`eric`), a
  superuser who owns all 68 tables in this database, so a grant to the owner grants
  nothing. `pg_class.relacl` in `paperless` holds zero rows, which is the
  measurement proving no grant here ever took effect. Full 54-statement audit in
