...
import { STEPS, calculateRange, getLabel, getAttribution, fireEvent, fmt, WEBHOOK } from "./deckData";
...
export default function DeckCalculator() {
  ...
  const submitLead = async () => {
    ...
    const payload = { action: "calculator_lead_full", contact, projectState: state, estimate: { low: lo, high: hi }, timestamp: new Date().toISOString(), source: "website_calculator", calculator: "deck", source_page: window.location.pathname, ...getAttribution() };
  ...
  return (
    <CalculatorLayout sidebar={<DeckEstimateSidebar state={state} step={step} />} lo={lo} hi={hi} step={step} totalSteps={STEPS.length} fmt={fmt} phase={phase}>
      {phase === "quiz" && (
        ...
        {phase === "results" && (
          <div>
            ...
            <div style={{ background: T.copperLight, border: `1px solid ${T.copperMid}`, borderRadius: 12, padding: 24 }}>
              ...
              <div style={{ fontSize: 13, color: T.textMuted, marginBottom: 18, lineHeight: 1.6 }}>A breakdown of your selections, material considerations for Bozeman's climate, and what to expect next. Eric will be in touch to answer any questions.</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <textarea placeholder="Anything else about the project? (optional)" value={contact.notes} onChange={(e) => setContact((c) => ({ ...c, notes: e.target.value }))} rows={2} style={{ ...inputStyle, resize: "vertical" }} onFocus={(e) => (e.target.style.borderColor = T.copper)} onBlur={(e) => (e.target.style.borderColor = T.border)} />
                <div>
        ...
        {phase === "submitted" && (
          <div style={{ textAlign: "center", padding: "32px 0" }}>
            ...
            <p style={{ fontSize: 14, color: T.textMuted, maxWidth: 400, margin: "0 auto", lineHeight: 1.6 }}>
              ...
              <strong style={{ color: T.copper }}>{fmt(lo)}–{fmt(hi)}</strong> is based on real Bozeman deck costs — Eric will be in touch to discuss your project.
            </p>
            <div style={{ margin: "24px auto 0", padding: "16px 20px", borderRadius: 12, background: T.copperLight, display: "inline-block", textAlign: "left" }}>
              ...
              <div style={{ fontSize: 13, color: T.text, lineHeight: 1.8 }}>1. Project summary lands in your inbox<br />2. Eric calls to discuss your project<br />3. Site visit — measurements, conditions, access<br />4. Detailed estimate within 48 hours</div>
            </div>
            {reportUrl && (
