...
import { STEPS, calculateRange, getLabel, getAttribution, fireEvent, fmt, WEBHOOK, WEBHOOK_APPT } from "./bathroomData";
...
export default function BathroomCalculator() {
  ...
  const goNext = () => fade(() => {
    ...
    if (n >= STEPS.length) {
      ...
      fireEvent("calculator_complete", {
        ...
        bathroom_size: state.bathroom_size,
  ...
  const pick = (id, v) => {
    if (step === 0 && Object.keys(state).length === 0) {
      fireEvent("calculator_started", { calculator_type: "bathroom" });
    }
    setState((p) => ({ ...p, [id]: v }));
  ...
  return (
    <CalculatorLayout sidebar={<EstimateSidebar state={state} step={step} phase={phase} />} lo={lo} hi={hi} step={step} totalSteps={STEPS.length} fmt={fmt} phase={phase} title="Bathroom Remodel Cost Calculator">
      {/* Progress bar */}
      {phase === "quiz" && (
        ...
        {phase === "gate" && (
          <div style={{ maxWidth: 480, margin: "0 auto" }}>
            <div style={{ textAlign: "center", marginBottom: 32 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: T.textMuted, letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 12 }}>Your bathroom remodel estimate</div>
              <div style={blurStyle}>
