...
## Changelog
- 2026-09-19: Calculator submissions carry the per-tab `visit_id` that the
  site's `tracking.js` mints, read from `sessionStorage` in
  `calculator/app/src/calcData.js` alongside the existing referrer /
  landing-page / pages-viewed attribution. hwc-crm uses it to show which pages a
  lead looked at (Umami). One field on the existing payload; the bundle needs a
  rebuild + a `?v=` bump per the cache-bust gotcha below.
