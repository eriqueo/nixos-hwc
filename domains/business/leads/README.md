...
- **2026-08-11** — **Case identity before write (hwc-crm D33, engineering-principles P21 law 1).** This service was the un-fingerprinted half of a two-writer table: it minted a fresh uuid per submission, so one person submitting the calculator twice became two cases and got every automated email twice (Chelsea Heveran, 2026-07-26/28). `store.save()` is now resolve-then-insert in a single transaction against `hwc.lead_fingerprint(email, phone)`, and returns the **resolved** `leadId`; `main.ts` addresses the report FK, JT graph, notify ping, customer email and response body to that id rather than to the freshly-minted one, which on a duplicate names a row that was never written. A same-source resubmission inside 48h skips the T+0 ack (`DUP_ACK_WINDOW_HOURS`) — bounded, not permanent, so a returning customer with a genuinely new project still gets their summary. `main()` became async so the new boot-time `store.verifySchema()` resolves **before** `server.listen`: the fingerprint function lives in hwc-crm's migration 009, which this service neither owns nor applies, and running without it does not error — it silently resumes forking cases. **Deploy order: apply hwc-crm 009 first, or this service exits 1 at boot by design.** `SaveResult` is a tagged union (`inserted` | `matched`) rather than `inserted: boolean` + optional `existing`: the boolean shape permitted "neither" — no fingerprint match *and* a no-op insert — which callers read as a fresh lead and re-acknowledged. Since `buildLead` server-fills the id, that combination means a uuid4 collision, so the adapter now throws instead of guessing whether the capture was already acknowledged. Files: `src/ports/store.ts` (`SaveResult`, `verifySchema`), `src/adapters/store-postgres.ts` (`RESOLVE_CASE_SQL`, `saveOnce`, one 23505 retry for the insert race), `src/main.ts`.
- **2026-07-29** — Comment-only: `src/schemas/lead.ts` principle citation updated from the retired `creating-systems.md §4` split-doc section to rev 3 numbering (Principle 2, Parse Don't Validate) in the single canonical engineering-principles doc. No behavior change.
