...
export function EstimateTab({ groups, totals, overrides, setOverrides, removed, setRemoved, onBack, onDetails, onOpenBrowser, state, set, isMobile = false }) {
  ...
  const removePick = (pickIndex) => {
    const picks = [...(state.catalog_picks || [])];
    picks.splice(pickIndex, 1);
    set('catalog_picks', picks);
  };
        ...
        {isMobile ? (
          <>
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={onOpenBrowser} style={{ ...ghostBtn, flex: 1, color: C.blu, borderColor: C.blu }}>+ Catalog</button>
            </div>
            <button
          ...
          <>
            ...
            <button onClick={onOpenBrowser} style={{ ...ghostBtn, color: C.blu, borderColor: C.blu }}>+ Catalog</button>
