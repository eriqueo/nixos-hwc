...
├── crm/               # hwc.business.crm — front-of-funnel CRM on :11660
├── databases/         # hwc.business.databases — business PostgreSQL layer
├── datax/             # Unreferenced 2026-05 scraper leftovers (no module; see 2026-08-26 below)
...
- 2026-09-05: crm — new `hwc.business.crm.controlTokenSecretRef` (agenix
  ...
  hwc-control-bot. It authorizes nothing else — no sends, sequences, JT, or stage
  forces — and `null` leaves those routes answering 503 (fail closed). An
  assertion rejects a ref that isn't a declared agenix secret (`d7b055fb`).
- 2026-08-29: morning-briefing email subject is decision-first — `N need you`
  (from `sections.today.items`, capped at 5) takes precedence over `N to watch`
  (alerts), instead of leading with a raw alert count (`7f332a78`). Same day, the
  CEO information contract landed across the briefing gatherer, dashboard and
  run script (`dc0fce28`).
