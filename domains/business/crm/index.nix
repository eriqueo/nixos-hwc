...
let
  ...
  crmWrapper = pkgs.writeShellScript "hwc-crm-wrapper" ''
    ...
    ${lib.optionalString cfg.calendar.enable ''
      ...
      export HWC_CRM_BUSY_FEEDS="${
        lib.concatStringsSep "," (
          lib.mapAttrsToList (label: ref: "${label}=${config.age.secrets.${ref}.path}") cfg.calendar.busyFeeds
        )
      }"
    ''}
...
in
{
  ...
  options.hwc.business.crm = {
    ...
    leadscoutIngest = {
      ...
      routesJson = lib.mkOption {
        type = lib.types.str;
        readOnly = true;
        internal = true;
        default = builtins.toJSON {
          version = 2;
          since_days = cfg.leadscoutIngest.sinceDays;
          routes = map (r: {
            profile = r.profile;
            pipeline = r.pipeline;
            source = r.source;
        ...
        description = "Derived v2 Lead Scout handoff contract consumed by the CRM ingest and Scout UI.";
      };
  ...
  config = lib.mkIf cfg.enable {
    ...
    assertions = [
      ...
      }
      {
        assertion =
          let
            prefixes = map (r: r.emailPrefix) cfg.leadscoutIngest.routes;
          in
          builtins.length prefixes == builtins.length (lib.unique prefixes);
    ...
    systemd.services.hwc-crm-leadscout-ingest = lib.mkIf cfg.leadscoutIngest.enable {
      ...
      environment = {
        ...
        HWC_CRM_INGEST_ROUTES = cfg.leadscoutIngest.routesJson;
      };
      serviceConfig = {
        Type = "oneshot";
        User = lib.mkForce cfg.user;
        Group = "users";
        ExecStart =
          "${pythonEnv}/bin/python3 -m hwc_crm.integrations.leadscout_ingest"
    ...
    systemd.services.hwc-crm-rolodex-sync = lib.mkIf (cfg.calendar.enable && cfg.rolodex.enable) {
    ...
    systemd.timers.hwc-crm-rolodex-sync = lib.mkIf (cfg.calendar.enable && cfg.rolodex.enable) {
      wantedBy = [ "timers.target" ];
      timerConfig = {
