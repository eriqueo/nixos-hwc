...
- **2026-06-29** — Stop concatenating the `is-system-running` fallback into the previous probe's output (4d2fc19e): the missing `else` branch joined stale text onto each new sample and broke the `system.overall` classifier. Bash-only fix; no schema change.
