...
- **2026-07-06** — **6am email delivery + config-drift observability.** `run.sh` gained a Step 5 that renders `briefing.json` to plain text and mails it via msmtp (Proton `office@` alias as the From — a self-sent eric→eric mail gets auto-archived by Proton and never hits the Inbox); unit PATH gained msmtp/pass/gnupg/git. Added a computed `config_drift` section (HEAD vs deployed rev, unpushed/dirty counts, reboot-pending via booted-vs-current kernel, generation count, 24h coredumps) with per-gap warning alerts; deployed rev read via `nixos-version --configuration-revision` (the `/run/current-system` revision file doesn't exist on this release) and flake glue now records `system.configurationRevision`. `hwc_morning_status` rewritten as a pure reader of `briefing.json` (one-producer doctrine). Also fixed the `is-system-running` fallback that concatenated `degraded\nunknown` on a degraded system.
