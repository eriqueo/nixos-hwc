...
## Changelog
- 2026-08-24: **Motion tuned so `record.retain.mode = "motion"` actually thins
  anything.** Measured against the live recordings DB: every segment carried
  motion > 0 — cobra_cam_1 logged 23.1 motion hours out of 23.1 recorded, reolink
  24.0 of 24.0, while real objects appeared in only 0.3 and 1.6 hours. At Frigate's
  defaults (`threshold 30`, `contour_area 10`, `improve_contrast true`) sensor
  noise registers as motion, so the motion retention mode thinned nothing and the
  3-day window held the full ~495 GB of continuous footage. The per-camera motion
  masks never covered this — they mask regions, not noise-sized blobs.
  `parts/config.nix` gains a global motion block: `threshold 40`,
  `contour_area 40` (minimum blob size in pixels), `improve_contrast false`
  (Frigate's documented cause of constant night motion). Object detection is
  unaffected — alerts are object-and-zone gated, not motion gated. Verified in the
  generated `frigate-config.yml` (`4a1d5afa`).
