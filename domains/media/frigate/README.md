...
## Changelog
- 2026-08-24: **Global motion block added so `record.retain.mode = "motion"` actually thins recordings.** Measured that day against the live recordings DB, every segment carried motion > 0 — cobra_cam_1 logged 23.1 motion hours out of 23.1 recorded and reolink 24.0 of 24.0, while real objects appeared in only 0.3 and 1.6 hours. At Frigate's defaults (`threshold` 30, `contour_area` 10, `improve_contrast` true) sensor noise counts as motion, so the motion-mode retention thinned nothing and the 3-day window held the full ~495 GB of continuous footage; the per-camera motion masks never covered this, since they mask regions rather than noise-sized blobs. `parts/config.nix` now sets `motion = { threshold = 40; contour_area = 40; improve_contrast = false; }` — `contour_area` is the minimum blob size in pixels and `improve_contrast` is Frigate's documented cause of constant night motion. Object detection is unaffected (alerts are object-and-zone gated). Verified only that the generated `frigate-config.yml` carries the block; the motion fraction still needs re-measuring after a container restart.
