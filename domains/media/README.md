...
## Changelog
- 2026-09-09: recyclarr — explicit `max`/`preferred` size caps added to the Sonarr and Radarr quality definitions, and **recyclarr established as the single writer of that endpoint**. The TRaSH guide data sets only a per-tier *minimum* and leaves max unlimited by design — it gates quality with custom formats, not size — so both apps had been taking whatever the indexer offered. Measured: Parks and Recreation held 122 WEBRip-1080p files at a median 2.2 GB for a 21-minute episode (100 MB/min) while a 1.01 GB WEBDL-1080p of the same episode sat in the release list, rejected only by the upgrade-only rule. The Radarr half of this was supposedly fixed on 2026-08-24 by `radarr-rightsize.py --set-max-size` writing to the API directly; **recyclarr reverted all twelve values on its next nightly run and every run since**, and all twelve measured `None` again on 2026-09-09. Nothing surfaced because recyclarr logs the reset as a successful sync — the failure mode of two writers on one field is silence, not conflict. That subcommand now refuses and names this file. Values are derived from the p90 (`max`) and median (`preferred`) of what the library already holds per tier, measured across 4,300 episode files and 323 movies, so the cap removes outliers rather than the normal case; the 1.7 GB AMZN WEB-DL grabbed the same evening measures 77 MB/min and stays legal under the 85 cap. Every `max` was checked against its tier's live guide minimum before writing, and the minimums are recorded inline — a `max` at or below `min` does not cap a tier, it makes the tier ungrabbable, which is how four Radarr tiers broke in August. Remux tiers are left uncapped deliberately: they sit outside the HD-1080p profile, so a cap there gates nothing and only risks that inversion.
- 2026-09-09: sabnzbd — `download_dir` and `complete_dir` forced absolute, joining `url_base` and `host_whitelist` in the existing `ExecStartPre` enforcer. A stock `sabnzbd.ini` ships both as **relative** paths (`Downloads/incomplete`, `Downloads/complete`), which SABnzbd resolves against its own home — `/config`, i.e. `/opt/sabnzbd` on the 468 GB NVMe **root** disk — not the 916 GB hot pool that `/downloads` maps to. Half of this was masked: every real category (`movies`, `tv`, `music`, `books`) carries an absolute `dir`, so completed files always landed correctly and nothing looked wrong. The **incomplete** path has no category override, so every usenet download staged on the system disk and only moved to the hot pool on completion — confirmed by `/opt/sabnzbd/config/Downloads/incomplete` being written at 21:55 during this session's Sonarr re-grab. Root had 103 GB free against a 916 GB pool that should have been absorbing it; a single large download could have filled the system disk. New options are `downloadDir`/`completeDir` rather than literals, because the container path is a deployment fact — the same reasoning that kept `books.httpRoot` as an option set to `""` on 2026-08-20. Extended the enforcer that already existed instead of adding a second writer to the same file; note its start-only caveat in the 2026-08-19 entry still applies.
- 2026-09-09: recyclarr — image pinned to `8.7.2` and the `image` option actually consumed. Upstream stopped publishing `ghcr.io/recyclarr/recyclarr:latest`, so `recyclarr-sync.service` failed every night from at least 2026-09-06 with `manifest unknown` (exit 125, before the container ever ran). The failure was silent in effect, not in the journal: the timer's `OnFailure` fired each night, but nothing downstream notices that custom formats and quality-profile scores have stopped being synced — the *arr apps keep scoring releases against whatever they last received. Discovered while auditing why Sonarr grabbed a fake `.exe` release for a new episode; frozen scoring is the layer that should reject that. Two defects, not one: `index.nix` defaulted `image` to `:latest`, **and** `parts/config.nix:188` hardcoded the same string in `ExecStart`, so the option was dead code and pinning it alone would have changed nothing. `:latest` on a third-party image is an unpinned dependency wearing a version's clothes — it fails on the vendor's schedule, not ours. Bump the pin deliberately.
...
- 2026-08-20: navidrome + books — URL bases cleared for the vhost move (`ND_BASEURL` `/music` → `""` in `navidrome-container/sys.nix`; `hwc.media.books.httpRoot` default `/books` → `""`). Both are the app half of a route flip that cannot be split across rebuilds, because each app fails closed at root while its base is set — navidrome 302s, LazyLibrarian 404s. `httpRoot` was kept as an option set to `""` rather than removed: the prefix is a deployment choice, not a property of LazyLibrarian, and deleting the knob would make putting it back a code change instead of a config change.
