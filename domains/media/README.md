...
## Changelog
- 2026-08-03: `immich-container/README.md` refreshed — header and Structure
  still described the pre-migration `domains/server/containers/immich/` layout
  with an `options.nix` that no longer exists, and the 2026-03-29 mount swap
  (`/mnt/media/pictures` → `${hwc.paths.photos}/external`) was unlogged. Docs
  only.
