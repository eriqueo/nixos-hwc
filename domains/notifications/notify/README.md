...
- **2026-07-13 (firefly automation)**: Added `parts/routes.nix` rules for the Firefly III build-out — a `finance-to-alerts` route so `topic=finance` (the daily `firefly-digest.timer`) lands in `#hwc-alerts`.
- **2026-07-12 (#media + #frigate channels)**: New `discord-media` / `discord-frigate` channels (`parts/channels.nix`, secretRefs `discord-webhook-media` / `discord-webhook-frigate`). `topic=media` re-homed out of `#hwc-alerts` into `#media` (media grabs are informational, not ops alerts); new `topic=frigate` route carries camera-health alerts to `#frigate`. n8n frigate-detect detection events post to the same `#frigate` webhook directly (they need snapshot image uploads the dispatcher doesn't do).
- **2026-07-12 (alert actionability audit)**: `parts/routes.nix` — `p1-fanout` no longer pages `#hwc-leads` (ops criticals were landing in the leads-only channel). `core/from-alertmanager.ts` now parses the P-scale severities (P5=Critical/P4=Warning/P3=Info) so alertmanager alerts no longer all collapse to priority 3. (Broader audit also touched prometheus/monitoring modules outside this domain.)
- **2026-07-07 (notification unification, M2–M5)**: `parts/routes.nix` additions completing the Slack/gotify eradication — one dispatcher, no path touches Slack or gotify. (Removals of `slack-webhook.nix`, `health.nix`, the `webhook.*` options, and the n8n `sys:router:notify` retirement live outside this domain.)
