...
- **2026-07-12 (#media + #frigate channels)**: Added `parts/channels.nix` channels for `#media` and `#frigate` (agenix `discord-webhook-media` / `discord-webhook-frigate`). `topic=media` leaves `#hwc-alerts` for `#media`; a new `topic=frigate` route in `parts/routes.nix` carries the camera-health alerts.
- **2026-07-12 (alert audit — every alert actionable)**: Killed false-firing and silent-noise paths across `parts/routes.nix` and the alertmanager adapter. `from-alertmanager.ts` now parses the P-scale (P5=Critical/P4=Warning/P3=Info) instead of collapsing every alertmanager alert to priority 3. `routes.nix` `p1-fanout` no longer pages `#hwc-leads` (ops criticals were landing in the leads-only channel). Companion Frigate-alert threshold/label fixes live in the monitoring domain.
- **2026-07-07 (notification unification, M2–M5)**: Slack/gotify fully eradicated — hwc-notify is the sole path. `hwc-alert` CLI POSTs native `NotificationInput` to `:11600/notify` and `slack-webhook.nix` was deleted; smartd/service-failure/backup notifiers in `notify-scripts.nix` became thin `hwc-alert` wrappers. Deleted `health.nix` (obsolete n8n-webhook health timer), the `webhook.*` options, and the n8n-required/baseUrl assertions. New `canary.nix`: a daily deadman probe over Discord + SMTP that fails loud (non-zero unit + sentinel + wall) on any adapter drop. n8n `sys:router:notify` retired; jellyfin + workflows now POST `:11600` directly (frigate-detect + bozeman-approval briefly kept on Slack as the migration tail).
