...
## Changelog
- 2026-07-13: Static-vhost `api` proxy fix — the served half is now wrapped in a sibling `handle` instead of a bare `try_files`. Bare `try_files` is a rewrite and runs before `handle` in Caddy's directive order, so `POST /mcp` was rewritten to `/index.html` and `file_server` answered 405. Sibling handles are mutually exclusive, most-specific path wins. Follow-up to the 2026-07-12 `api`-attr addition.
