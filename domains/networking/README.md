...
## Changelog
- 2026-06-16: Eval-time guard against route conflicts in `reverseProxy/index.nix`. Four assertions now fire when `hwc.networking.shared.routes` contains: a duplicate route `name` (any mode), a duplicate `name` among `mode = "vhost"` routes (subset, kept explicit for clear error), a duplicate `path` among `mode = "subpath"` routes, or a duplicate `port` among `mode = "port" | "static"` routes (the listener-port collision the recurring-mistakes list called out). Messages name the colliding routes. Upstream URL duplicates are deliberately **not** asserted — one service legitimately appears under multiple routes (e.g. n8n at port 2443 + `/webhook` subpath, jellyseerr vhost + subpath, lead-scout vhost + `lead-scout-api` port). Proven by injecting a duplicate name on a temporary commit (eval failed with the new message), then reverting; `nix flake check` is green on the released state.
