...
## Changelog
- 2026-06-11: `tasks/` — Phase C plumbing (`8d00b4e2`): Radicale CalDAV backend added as primary sync target. Post-Apple-Reminders-upgrade (`8581edb6`) the iCloud pair is dead; `hwc.mail.tasks.icloud.enable` gates it (default true, off on laptop) and Radicale is now canonical. Also: tasks pair `conflict_resolution` + metasync after sync (`b4f24ce4`), Phase A GO result + collections gotcha recorded (`e178c1c5`), pin tasks pair to VTODO collections only (`885fc96a`). SUPER+E aerc keybind fix + alias aerc to server aerc on laptop (`1a78f22d`).
