...
aerc/
  ...
  package.nix            # aerc from the forked github:eriqueo/aerc flake input
  parts/
    tags.nix             # Tag definitions, derived from domains/mail/taxonomy/
    ...
    appearance.nix       # hwc-theme styleset + which-key card colours (palette-driven)
...
- 2026-08-17: Structure block corrected — `theme.nix`, `session.nix` and `behavior.nix`
  no longer exist; `appearance.nix`, `sieve-filters.nix`, `tags-custom.json` and
  `package.nix` were missing.
- 2026-07-16: `b4f466e6` — `parts/config.nix` repointed at the new
  `domains/mail/contacts/` module (khard + a CardDAV pair on the CRM rolodex) for
  address completion.
- 2026-07-09: `d31fdad3` — triage folders + set-bucket keybinds (unified-triage
  Phase 2): `parts/tags.nix` +12, `parts/config.nix` +17 (tree-nested, inbox-scoped
  `triage/*` virtual folders), `parts/binds.nix` +23 (`<Space>tu/tr/tn` set-bucket,
  `<Space>gU/gR/gN` go-to). Replace-set semantics match the MCP gateway's
  `hwc_mail set-triage`.
- 2026-07-09: `02c72431` — `parts/tags.nix` shrank 91 → 31 lines (-60) by deriving from
  the new canonical `domains/mail/taxonomy/` registry instead of restating the tag
  vocabulary. The "single source of truth for tag definitions" is now taxonomy; this
  file is the aerc projection of it. The **Current Tags** table above is therefore a
  snapshot of taxonomy output, not the definition.
