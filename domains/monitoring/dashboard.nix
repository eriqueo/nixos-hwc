...
#
# USED BY (Downstream):
#   - profiles/server.nix (enables via hwc.monitoring.dashboard.enable)
#
# IMPORTS REQUIRED IN:
#   - profiles/server.nix: ../domains/services/business/dashboard.nix
#
# USAGE:
#   hwc.monitoring.dashboard.enable = true;
#   hwc.monitoring.dashboard.port = 8501;
...
let
  cfg = config.hwc.monitoring.dashboard;
...
in {
  ...
  options.hwc.monitoring.dashboard = {
