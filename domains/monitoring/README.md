...
└── heartbeat/          # Outbound ping to an external dead-man's switch
    └── index.nix       #   (healthchecks.io); the watcher OFF this host
...
- 2026-09-19: prometheus — new blackbox job `probe-crm-intake` and alert `CrmIntakeDown`. It sends a CORS preflight to `crm.iheartwoodcraft.com/hooks/contact` and `/hooks/calculator`, so it proves Cloudflare → tunnel → hwc-crm end to end and touches no data: it cannot create a lead or add to any count. The website's calculators and contact forms now post to hwc-crm; the older `probe-webhook-ingress` and `probe-leads-service` stay until the hwc-leads path is deleted.
...
- 2026-08-15: **Exportarr + blackbox probes de-pinned from the \*arr URL bases.** The five \*arr apps moved from subpath to name-based vhosts and now run with an empty URL base (see the networking and media READMEs), which invalidates every loopback URL that carried the prefix. Rewired here: `exportarr/index.nix` `appUrls` (`http://127.0.0.1:8989/sonarr` → `:8989`, ×4) and `prometheus/index.nix` `httpServices` (5 probes, `/sonarr/ping` → `/ping`, including readarr which has no exporter). Left unfixed these would have failed quietly in the way this domain has been bitten by before: exportarr would export zero metrics rather than erroring, and the blackbox probes use the `http_reachable` module whose "up" set deliberately includes 4xx — so a 404 from the retired prefix would have **still reported the service as up**, leaving the Service Health dashboard green across five dead probes. The probe list being single-source-of-truth in Nix is what made this a ten-line change rather than a hand-audit of drifted monitors.
- 2026-07-12: **Frigate alert rules fixed (were false-by-design) + Grafana createHome clobber.** `FrigateLowFPS` fired permanently — cameras detect at 2–5 fps by design but the threshold was a hardcoded `< 10`; now relative to each camera's own 1-day baseline (`< 0.5 * avg_over_time[1d]`). `FrigateCameraOffline` fired forever for the deliberately-disabled cobra_cam_2 (0 fps); now requires the camera to have been alive over the last day. Both annotations used `{{ $labels.camera }}` but the exporter label is `camera_name` — Discord messages showed a blank camera name. Severity note: the rules' P-scale (P5=Critical/P4=Warning/P3=Info) was previously unintelligible to hwc-notify's `severityToPriority` (everything collapsed to priority 3); the adapter now parses it, so P5 rules genuinely page (p1-fanout → email). Separately, `grafana/index.nix` adds `users.users.grafana.createHome = mkForce false` — upstream's `createHome=true` re-chowned `/var/lib/hwc/grafana` to `grafana:grafana 0700` on every rebuild while the unit runs as eric, which had Grafana serving 503s with a dead alert scheduler since Jul 10.
