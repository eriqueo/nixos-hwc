...
#
# Hardware profile definitions for AI workloads
...
let
  ...
  detectedProfile =
    if cfg.selected != "auto" then cfg.selected
    else if (!hasGPU) then "cpu-only"
    else if (totalRAM_GB >= cfg.detection.ramThresholdGB) then "server"
    else "laptop";
  ...
  profiles = {
    ...
    laptop = {
      ...
      models = {
        small = "llama3.2:1b";    # 1.3GB
        medium = "llama3.2:3b";   # 2.0GB
        large = "phi3.5:3.8b";    # 2.3GB
      };
    ...
    server = {
      ...
      # Model selection by task complexity
      models = {
        small = "llama3.2:3b";       # 2.0GB - server can handle this as "small"
    ...
    cpu-only = {
      ...
      models = {
        small = "llama3.2:1b";    # 1.3GB
        medium = "llama3.2:1b";   # Force small for medium tasks too
        large = "llama3.2:3b";    # Max out at 3b
      };
