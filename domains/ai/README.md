...
- 2026-09-14: Law-12 refresh of the four sub-module READMEs. No code change. The
  drift they had accumulated: `agent/` gained a Structure + Changelog and lost an
  integration snippet naming a `domains/ai/default.nix` that no longer exists;
  `mcp/` dropped `heartwood/` from its Structure (deleted 2026-05-21 — JT is now a
  stdio backend of `domains/system/mcp/`, not the `domains/business/mcp/` the
  commit message predicted); `personas/` picked up `library/_defaults.nix` and the
  `import ./_defaults.nix // { … }` authoring form; `profiles/` dropped the removed
  `options.nix` and gained `parts/definitions.nix`.
