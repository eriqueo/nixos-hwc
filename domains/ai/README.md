...
- 2026-07-27: Law 12 changelog backfill — the `agent/`, `mcp/`, `personas/`, and `profiles/` subdomain READMEs were refreshed to catch up their `## Changelog` sections with already-committed code (tailnet sweep, heartwood-MCP relocation, persona-daemon schema, options inlining). Docs only; no functional change in this domain.
