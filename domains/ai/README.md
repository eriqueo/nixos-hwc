...
- 2026-08-17: Law 12 freshness pass — no module changes. Brought the four sub-domain READMEs (`agent/`, `mcp/`, `personas/`, `profiles/`) current with their code: `agent/` gained the Structure + Changelog sections it never had, and `mcp/`/`profiles/` had Structure blocks still listing `heartwood/` and `options.nix`, both deleted in 2026-05-21 (`4f199955`).
