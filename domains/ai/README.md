...
- 2026-07-06: Law 12 child-README sweep — refreshed the `agent/`, `mcp/`, `personas/`, and `profiles/` module READMEs to match current code (options inlined into each `index.nix`, `mcp/heartwood/` removal, persona-daemon RAG fields, dropped per-eval "AI Profile" warning). No `ai/`-index code change; this entry records the sub-tree doc refresh.
