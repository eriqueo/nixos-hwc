...
- 2026-06-10: shell/index.nix — snix/tnix/bnix refactored onto a shared `_hwc_rebuild` helper that tees rebuild output to a temp log and re-prints deduped `warning:`/`evaluation warning:`/`trace:` lines at the end, so new deprecation warnings can't scroll away unnoticed (zero-warning-baseline habit). `snix` additionally checks for a dirty git tree first and prompts before continuing (commit-before-rebuild doctrine).
