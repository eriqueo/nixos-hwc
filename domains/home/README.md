...
## Changelog
- 2026-09-12: Chromium, Proton Mail, T3 Code's UI subprocesses, and the dictation overlay now consume narrow integrated-rendering boundaries so routine desktop apps cannot keep NVIDIA awake; explicit `gpu-offload` use remains unchanged.
...
- 2026-08-30: bumped `apps/herdr/parts/package.nix` from v0.6.2 to v0.8.2 (upstream latest stable, 2026-08-19). Only `version` and `hash` changed; the fetchurl + autoPatchelfHook derivation is unchanged. Affects both hosts — laptop via `profiles/base/home.nix`, server via `machines/server/home.nix`.
