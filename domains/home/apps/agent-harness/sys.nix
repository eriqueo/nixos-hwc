...
let
  ...
  managedClaudeSettings = pkgs.runCommand "claude-managed-settings.json" { nativeBuildInputs = [ pkgs.jq ]; } ''
    jq '
...
in
{
  options.hwc.system.apps.agent-harness.enable = lib.mkEnableOption "machine-wide agent harness policy";
