...
- **2026-06-15 (b)**: Keymap comes from the unified factory — `index.nix` reads
  `hwc.home.keymap.grammar` and appends the `[keybindings]` block generated by
  `keymap/parts/to-khalt.nix`, so khalt gets the same list-app bare verbs as
  todui (`a` add, `e` edit, `d` delete, Enter open). Guarded: khalt still
  evaluates when the keymap module is not imported.
