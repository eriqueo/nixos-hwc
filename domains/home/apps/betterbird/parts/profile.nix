    ...
    ".local/bin/mail-gui" = {
      ...
      text = ''
        ...
        if [ "$FIRST_RUN" = "true" ]; then
          ...
          echo "1. Install tbkeys-lite addon for custom keybindings"
          echo "2. Gmail accounts will prompt for OAuth2 login (use regular passwords)"
          echo "3. Proton accounts will use Bridge passwords (generated by Bridge)"
          echo "4. Check unified folders and smart folders in sidebar"
          echo "5. All accounts configured for IMAP-only (no shared ~/Maildir)"
          echo ""
        fi
