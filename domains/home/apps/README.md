...
├── tasq/           # VTODO task TUI (syncs to Apple Reminders via vdirsyncer)
└── ... (30+ apps)
```
...
- 2026-06-12: Added claude-code `shareConfig` (`57834537`) — `~/.claude` skills/agents/commands/CLAUDE.md are now symlinked from a standalone `~/.claude-config` repo so hwc-laptop and the headless hwc-server share a single skill set. Decoupled from the package `enable` so the server gets the symlinks without pulling the Nix binary or a missing Obsidian cert path.
