...
- 2026-06-12: Retired in-tree `tasq` → external **`todui`**. tasq was rewritten as a standalone, todoman-free project (own repo `~/dev/todui`, own VTODO engine on icalendar, own tests/packaging/flake) and is now consumed as the `todui` flake input. `domains/home/apps/tasq/` + `workspace/home/tasq/` deleted; replaced by the thin translator `domains/home/apps/todui/` (namespace `hwc.home.apps.todui`) that feeds todui the system theme palette + Radicale creds. Input is a live-dev `path:` for now (pin to a git rev before hwc-server). See `todui/README.md`.
