...
## Changelog
- 2026-07-11: `parts/appearance.nix` — `[filetype]` glob rules switched from
  `name =` to `url =` (yazi renamed the key upstream). The old key raised
  "at least one of url or mime must be specified", which dropped yazi to preset
  settings entirely. The mime rules were already correct.
