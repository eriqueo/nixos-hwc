...
## Changelog
- 2026-07-11: `parts/appearance.nix` — the twelve `[filetype]` glob rules now
  use `url =` instead of `name =`; yazi renamed that key upstream, and the
  stale spelling raised "at least one of url or mime must be specified", which
  dropped yazi to preset settings entirely. The mime rules were already correct
  (`67c01e5a`).
