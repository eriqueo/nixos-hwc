...
## Changelog
- 2026-07-11: `67c01e5a` — `parts/appearance.nix` filetype rules switched from the
  removed `name =` key to `url =` (12 rules: the `*.nix` / `*.{py,js,json,toml,yaml,yml}`
  / `*.rs` / `*.ts` / `*.md` / `*.lua` / `*.sh` / `*.{c,cpp,h,go}` colour rules, the
  `is = "orphan"` broken-link rule, and the executable rules). Yazi dropped `name` from
  its `[filetype]` schema, so every one of these was silently inert. Colours unchanged.
