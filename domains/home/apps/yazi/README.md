...
## Changelog
- 2026-07-11: `parts/appearance.nix` — `[filetype]` rules switched from `name=` to
  `url=`. yazi renamed that glob key, and the stale spelling raised "at least one
  of url or mime must be specified", which dropped yazi to preset settings
  wholesale. The `mime` rules were already correct (`67c01e5a`).
