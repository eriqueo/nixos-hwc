...
## Changelog
- 2026-07-11 (b): `parts/appearance.nix` — the 12 `[filetype]` glob rules moved
  from `name =` to `url =`, the key yazi renamed. The old key produced "at least
  one of url or mime must be specified", which dropped yazi to preset settings
  entirely; the `mime` rules were unaffected.
