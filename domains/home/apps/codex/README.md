...
## Changelog
- 2026-08-29: Added `sharedSkillSource` plus the selected `herdr` and `project-director` skill symlinks under `~/.codex/skills`; Claude, Pi, and Codex now consume one source rather than copied orchestration instructions. HM-only → `hms`.
