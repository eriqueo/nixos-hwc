...
## Changelog
- 2026-09-17: `shareHarness` (default follows `claude-code.shareConfig.enable`) links `~/.codex/hooks.json` from `~/.claude-config/codex/hooks.json`, renders `~/.codex/AGENTS.md` with `codex-agents-render`, and records hook trust with `codex-hooks-trust`. Both run at activation and after every claude-config sync merge. AGENTS.md is rendered, not committed: it is the Codex preamble, CLAUDE.md, and the PRIMER-DIGEST block, so it cannot drift from its sources and two hosts cannot conflict on it. A hand-written AGENTS.md found in place is kept once as `AGENTS.md.pre-render.bak`. Trust exists because Codex runs no hook until its hash is recorded in the host's config.toml, and that state is per host. `hooks-trust.py` asks the Codex app-server (`hooks/list`) and writes trust through it (`config/value/write`) for user hooks.json entries only. Measured: the hash does not depend on Codex version. Do not run `herdr integration install codex` against the link; herdr's v8 entry is already in the shared file. HM-only → `hms`.
