# domains/home/apps/pi/parts/package.nix
#
# pi coding agent (@earendil-works/pi-coding-agent), pinned at v0.80.7 to
# match the imperative install captured on datax-box (/home/projects/bin/pi).
#
# Vendored from nixpkgs pkgs/by-name/pi/pi-coding-agent/package.nix (0.80.2)
# rather than overridden, because hwc-server rides nixpkgs-stable (25.11)
# which does not carry pi-coding-agent at all. Bumping = update `version`,
# `src.hash`, `npmDepsHash` (nix flake prefetch github:earendil-works/pi/vX +
# prefetch-npm-deps on that tag's package-lock.json).
{
  lib,
  buildNpmPackage,
  fetchFromGitHub,
