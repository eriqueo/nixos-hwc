...
- 2026-06-26: which-key restyled to the HWC standard popup look (matching the aerc card): `preset = "modern"`, double border, `→` separator, no per-mapping icons, `show_help = false`, plus `WhichKey*` highlight groups generated from the theme palette in `parts/appearance.nix` — which retires the default red group colour (eff098eb).
