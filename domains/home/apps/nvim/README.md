...
- 2026-06-26: Configured `which-key.nvim` (was a bare `setup()`) for the HWC standard popup look — modern preset, double-line border, `key → desc` rows via `icons.separator`, no per-mapping icons. Themed the `WhichKey*` highlight groups from the palette (`parts/appearance.nix`: raised card bg, copper border/keys, inverted cream title chip, cool-accent groups) so the default red group colour is gone. Mirrors the aerc which-key card.
