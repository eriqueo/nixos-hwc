...
- 2026-07-13: which-key.nvim given the HWC standard popup look — modern preset, double-line border, `key → desc` rows (no per-mapping icons), and palette-themed `WhichKey*` highlight groups (copper border/keys, cream title chip) so the default red group colour is gone. Mirrors the aerc which-key card. Config in `parts/appearance.nix` + `parts/lua/core/plugins.lua`.
