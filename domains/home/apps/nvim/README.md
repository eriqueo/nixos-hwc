...
- 2026-06-29: which-key.nvim now gets the HWC standard popup look (8b8560f8) — modern preset, double-line border, `key → desc` rows, no per-mapping icons, and `WhichKey*` highlight groups themed off the palette (raised `bg3` card, copper border, inverted cream title chip, copper keys, cool-accent groups). The default red group colour is gone; mirrors the aerc which-key card.
