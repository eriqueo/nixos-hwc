...
├── README.md           # This file
└── parts/
    ├── appearance.nix  # Palette → highlight-group overrides (incl. WhichKey*)
    └── lua/
        ├── core/
...
- 2026-06-26: which-key.nvim given the HWC standard popup look. It was a bare `setup()`; now the modern preset with a double-line border and `key → desc` rows (`icons.separator`), no per-mapping icons. The `WhichKey*` highlight groups are themed from the palette in `parts/appearance.nix` — raised `bg3` card, copper border, inverted cream title chip, copper keys, cool-accent groups — which is what removes the default red group colour. Mirrors the aerc which-key card (`eff098eb`).
