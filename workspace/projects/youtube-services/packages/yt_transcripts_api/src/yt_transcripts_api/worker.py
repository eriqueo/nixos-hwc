"""
Separate worker entry point for processing transcript jobs.

Runs independently from FastAPI server to avoid N workers per uvicorn worker problem.
"""
...
from .workers.processor import TranscriptProcessor
import structlog
...
class WorkerService:
    """Worker service that processes transcript jobs"""

    def __init__(self):
        self.db_pool: DatabasePool | None = None
        self.processor: TranscriptProcessor | None = None
        self.shutdown_event = asyncio.Event()

    async def start(self):
        """Start the worker service"""
        # Configure logging
        configure_logging("yt-transcripts-worker", level=config.log_level)
        ...
        # Create processor
        self.processor = TranscriptProcessor(self.db_pool, config)
