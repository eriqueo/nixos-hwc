...
// Configurable webhook URL — set via localStorage or env
...
export function EstimateTab({ groups, totals, overrides, setOverrides, removed, setRemoved, onBack, onDetails }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 14, justifyContent: 'flex-end', flexWrap: 'wrap' }}>
        ...
        <button onClick={pushToWebhook} style={accentBtn}>
          {pushMsg === 'pushing' ? '...' : pushMsg === 'pushed' ? '✓ Sent' : pushMsg === 'error' ? '✗ Error' : 'Push to JT'}
        </button>
      </div>
