...
12. ~~Finish workbench's last mile~~ — **already done** (live verification: deployed, live session, real tile fetches). The 2026-09-04 boundary repair retired the stale handoff and staged Nix copies after comparison with the live implementation. Current operating instructions are in [Workbench README](https://github.com/eriqueo/workbench/blob/main/README.md#current-support-boundary) and [RUN.md](https://github.com/eriqueo/workbench/blob/main/RUN.md). The separate khalt test follow-up is outside this artifact cleanup.
