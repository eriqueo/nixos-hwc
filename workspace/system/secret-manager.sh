...
#   - Add: Create new encrypted secret (multi-recipient; auto-generated mount)
...
add_secret() {
  ...
  PS3="Category: "
  # caddy/ is intentionally excluded: those certs are managed via `tailscale cert`
  # + the hand-written parts/caddy.nix (runtime hostname selection), not this TUI.
  local categories=("services" "infrastructure" "home" "system")
  local category=""
  ...
  # Encrypt to all recipients (use printf to avoid a trailing newline)
  if printf '%s' "$secret_value" | encrypt_to_everyone "$secret_file"; then
    log_info "Encrypted secret saved (all hosts): $secret_file"
  else
    log_error "Failed to encrypt secret"
    return 1
  fi

  # Step 5: Stage the .age for flake visibility, then validate.
  # No declaration edit needed — the generated layer (domains/secrets/lib.nix)
  # auto-recipients and auto-mounts every parts/**.age from the filesystem.
  echo ""
  log_step "Staging .age for flake check..."
  (cd "${NIXOS_DIR}" && git add "$secret_file")
  log_info "Staged $secret_file"
...
edit_secret() {
  ...
  # Re-encrypt to all recipients (use printf to avoid a trailing newline)
  if printf '%s' "$new_value" | encrypt_to_everyone "$secret_file"; then
    log_info "Secret updated (all hosts): $secret_file"
    echo ""
...
validate_secrets() {
  ...
  # Check 5: Generator coverage
  # Declarations are GENERATED from parts/**.age (domains/secrets/lib.nix), so
  # there is nothing per-secret to drift. The parity harness asserts every .age
  # has a recipient rule + a mount and that the counts line up.
  log_step "5. Generator Coverage"
  local age_files
  age_files=$(find "${SECRETS_PARTS}" -name "*.age" -type f 2>/dev/null)
  if [[ -f "${NIXOS_DIR}/workspace/system/secrets-parity.sh" ]]; then
    if (cd "${NIXOS_DIR}" && bash workspace/system/secrets-parity.sh >/dev/null 2>&1); then
      log_info "All .age files are recipiented and mounted by the generator"
    else
      log_warn "Parity check reported issues — run: workspace/system/secrets-parity.sh"
    fi
  else
    log_info "Secrets are generated from parts/**.age (no per-secret declarations)"
  fi
  echo ""
