...
let
  ...
  wallpaperPath = "/home/eric/03-tech/nixos-hwc/assets/wallpapers/nord-mountains.jpg";
  ...
  systemHealthChecker = pkgs.writeScriptBin "system-health-checker" ''
    #!/usr/bin/env bash
    # Simple system health overview
    notify-send "System Health" "$(uptime)" -t 3000
  '';
  ...
  options.hwc.desktop.hyprland = {
    ...
    monitor = {
      primary = lib.mkOption {
        type = lib.types.str;
        default = "eDP-1,2560x1600@165,0x0,1";
        description = "Primary monitor configuration";
      };
      external = lib.mkOption {
        type = lib.types.str;
        default = "DP-1,1920x1080@60,2560x0,1";
  ...
  config = lib.mkIf cfg.enable {
    ...
    wayland.windowManager.hyprland = {
      ...
      settings = {
        # Monitor setup
        monitor = [
          cfg.monitor.primary      # Laptop at 0,0 (left)
          cfg.monitor.external     # External at 2560,0 (right)
        ];
