...
in
{
  ...
  # Improved resource monitor that actually outputs JSON
  ...
  battery-health = sh "waybar-battery-health" ''
    ...
    if   [[ ''$CAPACITY -gt 90 ]]; then ICON="󰁹"; CLASS="full"
  ...
  network-settings = sh "waybar-network-settings" ''
    ...
    case "''$CHOICE" in
      ...
      "VPN Status")                   kitty --title "VPN Status" -e sh -c 'echo "=== VPN Status ==="; echo ""; if command -v vpnstatus >/dev/null; then vpnstatus; else echo "VPN status command not found"; fi; echo ""; read -p "Press Enter to close..."' & ;;
