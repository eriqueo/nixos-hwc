# nixos-hwc/modules/infrastructure/waybar-hardware-tools.nix
#
# WAYBAR HARDWARE TOOLS - Complete hardware monitoring tools for Waybar integration
# Provides all 13 hardware monitoring scripts that Waybar custom blocks consume
#
# DEPENDENCIES (Upstream):
#   - config.hwc.infrastructure.gpu.enable (modules/infrastructure/gpu.nix)
#   - systemPackages for basic tools (pkgs.jq, pkgs.libnotify, etc.)
#
# USED BY (Downstream):
#   - modules/home/waybar/default.nix (consumes all 13 tool binaries)
#   - profiles/workstation.nix (enables infrastructure capability)
#
# IMPORTS REQUIRED IN:
#   - profiles/workstation.nix: ../modules/infrastructure/waybar-hardware-tools.nix
#
# USAGE:
#   hwc.infrastructure.waybarHardwareTools.enable = true;
#   # Provides: all 13 tools with dual naming (waybar-* + unprefixed wrappers)

{ config, lib, pkgs, ... }:

let
  cfg = config.hwc.infrastructure.waybarHardwareTools;
in {
  #============================================================================
  # OPTIONS - What can be configured
  #============================================================================
  options.hwc.infrastructure.waybarHardwareTools = {
    enable = lib.mkEnableOption "Complete hardware monitoring tools for Waybar";
  ...
  config = lib.mkIf cfg.enable {
    ...
    environment.systemPackages = with pkgs; [
      ...
      (writeShellScriptBin "waybar-resource-monitor" ''
        ...
        # CPU usage
        CPU_USAGE=$(${procps}/bin/top -bn1 | grep "Cpu(s)" | awk '{print $2}' | sed 's/%us,//')
