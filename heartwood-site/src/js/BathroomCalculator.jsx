    ...
    <div className="text-center py-12">
      ...
      <div className="mt-10 pt-6 border-t border-gray-200">
        <p className="text-gray-500">
          Questions? Call us at <a href="tel:4065515061" className="text-brand-600 font-medium">406-551-5061</a>
        </p>
      </div>
    </div>
  );
...
export default function BathroomCalculator() {
  ...
  const handleNext = () => {
    if (currentStep < totalSteps - 1) {
      // GA4: track step completion
      if (typeof gtag === 'function' && !isContactStep) {
        gtag('event', 'calculator_step', {
          step_number: currentStep + 1,
          step_id: step.key,
          selected_value: step.type === 'multi'
            ? (projectState[step.key] || []).join(',')
            : projectState[step.key]
        });
      }
      // GA4: track reaching results/contact step
      if (currentStep === STEPS.length - 1 && typeof gtag === 'function') {
  ...
  const handleSubmit = async () => {
    ...
    // UTM attribution from tracking.js sessionStorage
    try {
      const attr = JSON.parse(sessionStorage.getItem('hwc_attribution') || '{}');
    ...
    try {
      ...
      if (typeof gtag === 'function') {
        gtag('event', contact.email ? 'calculator_lead_full' : 'calculator_lead_basic', {
          estimate_low: estimate.low,
          estimate_high: estimate.high
        });
      }
  ...
  return (
    <div className="bg-gradient-to-b from-gray-50 to-white py-8 px-4">
      <div className="max-w-lg mx-auto">
        {/* Header */}
