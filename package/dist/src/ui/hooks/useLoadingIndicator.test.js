...
describe('useLoadingIndicator', () => {
    ...
    const renderLoadingIndicatorHook = (initialStreamingState) => {
        let hookResult;
        function TestComponent({ streamingState, }) {
            hookResult = useLoadingIndicator(streamingState);
            return null;
        }
        const { rerender } = render(_jsx(TestComponent, { streamingState: initialStreamingState }));
        return {
            result: {
                get current() {
                    return hookResult;
    ...
    it('should show waiting phrase and retain elapsedTime when WaitingForConfirmation', async () => {
        const { result, rerender } = renderLoadingIndicatorHook(StreamingState.Responding);
        await act(async () => {
