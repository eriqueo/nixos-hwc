...
import { setupGithubCommand, updateGitignore, GITHUB_WORKFLOW_PATHS, GITHUB_COMMANDS_PATHS, } from './setupGithubCommand.js';
...
describe('setupGithubCommand', async () => {
    ...
    it('throws an error when download fails', async () => {
        const fakeRepoRoot = scratchDir;
        const fakeReleaseVersion = 'v1.2.3';
        vi.mocked(global.fetch).mockResolvedValue(new Response('Not Found', {
            status: 404,
        ...
        await expect(setupGithubCommand.action?.({}, '')).rejects.toThrow(/Invalid response code downloading.*404 - Not Found/);
    });
});
