...
export async function parseArguments(settings) {
    ...
    const yargsInstance = yargs(rawArgv)
        ...
        if (argv.resume && !argv.prompt && !process.stdin.isTTY) {
            throw new Error('When resuming a session, you must provide a message via --prompt (-p) or stdin');
        }
...
export async function loadCliConfig(settings, sessionId, argv, cwd = process.cwd()) {
    ...
    const enableMessageBusIntegration = settings.tools?.enableMessageBusIntegration ?? false;
