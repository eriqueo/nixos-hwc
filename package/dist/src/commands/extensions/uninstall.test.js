...
describe('extensions uninstall command', () => {
    ...
    describe('handleUninstall', () => {
        ...
        it('should uninstall multiple extensions', async () => {
            ...
            expect(mockDebugLogger.log).toHaveBeenCalledWith('Extension "ext1" successfully uninstalled.');
            expect(mockDebugLogger.log).toHaveBeenCalledWith('Extension "ext2" successfully uninstalled.');
            expect(mockDebugLogger.log).toHaveBeenCalledWith('Extension "ext3" successfully uninstalled.');
            mockCwd.mockRestore();
        });
        it('should report errors for failed uninstalls but continue with others', async () => {
            ...
            expect(mockUninstallExtension).toHaveBeenCalledTimes(3);
            expect(mockDebugLogger.log).toHaveBeenCalledWith('Extension "ext1" successfully uninstalled.');
            expect(mockDebugLogger.error).toHaveBeenCalledWith('Failed to uninstall "ext2": Extension not found');
            expect(mockDebugLogger.log).toHaveBeenCalledWith('Extension "ext3" successfully uninstalled.');
        ...
        it('should exit with error code if all uninstalls fail', async () => {
            ...
            expect(mockDebugLogger.error).toHaveBeenCalledWith('Failed to uninstall "ext1": Extension not found');
            expect(mockDebugLogger.error).toHaveBeenCalledWith('Failed to uninstall "ext2": Extension not found');
            expect(mockProcessExit).toHaveBeenCalledWith(1);
            mockProcessExit.mockRestore();
            mockCwd.mockRestore();
        ...
        it('should log an error message and exit with code 1 when initialization fails', async () => {
            ...
            await handleUninstall({ names: ['my-extension'] });
            expect(mockDebugLogger.error).toHaveBeenCalledWith('Initialization failed message');
            expect(mockProcessExit).toHaveBeenCalledWith(1);
            mockProcessExit.mockRestore();
            mockCwd.mockRestore();
        });
    ...
    describe('uninstallCommand', () => {
        ...
        describe('builder', () => {
            ...
            it('check function should throw for missing names', () => {
                ...
                expect(() => checkCallback({ names: [] })).toThrow('Please include at least one extension name to uninstall as a positional argument.');
            });
        });
        it('handler should call handleUninstall', async () => {
            ...
            mockUninstallExtension.mockResolvedValue(undefined);
            const mockCwd = vi.spyOn(process, 'cwd').mockReturnValue('/test/dir');
            const argv = { names: ['my-extension'], _: [], $0: '' };
